...
def fetch_ohlcv(ticker: str, period: str = "1y", interval: str = "1d", drop_today: bool = False) -> tuple[pd.DataFrame, str]:
    ...
    for s in candidates:
        def _get(s=s):
            ...
            return df
        # 交易日 15:00 前今天的 K 棒可能還在變（盤中快取的半根 K 棒不能拿到盤後用），快取只留 15 分鐘
        now = pd.Timestamp.now()
        live = drop_today or (now.weekday() < 5 and now.hour < 15)
        df = cached(f"ohlcv:{s}:{period}:{interval}", 60 * 15 if live else 60 * 60 * 6, _get)
        if df is not None:
            df = df.dropna(subset=["Open", "High", "Low", "Close"])  # yf 週一的本週週 K 可能只有量、OHLC 為 NaN
        if df is not None and len(df):
