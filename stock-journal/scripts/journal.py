...
COLS = ["id", "code", "name", "entry_date", "entry_price", "qty", "thesis_type", "thesis", "falsify", "falsify_price", "falsify_ma", "falsify_level",
        ...
        "stop_init", "trail_stop"]   # stop_init＝進場時的停損（算 R 用，之後不改）；trail_stop＝移動停利（只上移）
...
def cmd_close(a):
    ...
    for k, v in zip(["status", "exit_date", "exit_price", "exit_reason", "pnl_pct", "held_days", "bench_pct", "excess_pct"],
                    ["closed", exit_date, a.price, a.reason, round(pnl, 2), held, bench, excess]):
        df.at[i, k] = v
    save(df)
    disc = ""
    ...
    rm = _r_mult(r, a.price)
    md_append(f"## #{a.id} {r['code']} {r['name']} 出場 {exit_date} @ {a.price}（{a.reason}）\n"
              f"- 損益 {pnl:+.2f}%" + (f"（{rm:+.2f}R）" if rm is not None else "") + (f"（同期大盤 {bench:+.2f}%，超額 {excess:+.2f}）" if bench is not None else "") + f"，持有 {held} 日（預計 {r['plan_days']}）{disc}\n- 備註：{a.note or '-'}")
    print(f"[close] #{a.id} {pnl:+.2f}%"); sync()
...
def _r_stats(cl: pd.DataFrame) -> str:
    ...
    return (f"- R 倍數（{len(rm)} 筆有初始停損）：平均 {rm.mean():+.2f}R，獲利單平均 {rm[rm > 0].mean() if (rm > 0).any() else 0:+.2f}R，"
            f"虧損單平均 {rm[rm <= 0].mean() if (rm <= 0).any() else 0:+.2f}R；虧損超過 1.2R 的 {len(over)} 筆" + ("（停損失守，看是跳空還是拖延）" if len(over) else "（停損有守住）"))
...
def cmd_review(a):
    ...
    out += ["", "## 逐筆", cl[["id", "code", "name", "thesis_type", "entry_date", "exit_date", "pnl_pct", "bench_pct", "excess_pct", "held_days", "plan_days", "exit_reason", "falsify_lag"]].to_markdown(index=False)]
...
if __name__ == "__main__":
    ...
    sp.add_parser("check"); p = sp.add_parser("review"); p.add_argument("--months", type=int)
